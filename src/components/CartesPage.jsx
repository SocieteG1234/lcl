...
const CartePage = () => {
  ...
  return (
    <div className="min-h-screen font-sans pb-24" style={{ backgroundColor: '#0d1b6e' }}>
      <div style={{ backgroundColor: LCL_BLUE }} className="pb-8 rounded-b-3xl">
        ...
        <div className="pt-2 pb-2">
          <div className="flex gap-4 overflow-x-auto px-6 pb-2 snap-x snap-mandatory">
            {CARDS_DEF.map((c, i) => (
              <button key={c.id} onClick={() => setCard(i)} className="snap-center flex-shrink-0 transition-all duration-300" style={{ width: '80vw', maxWidth: 320, transform: activeCard === i ? 'scale(1.04)' : 'scale(0.93)', opacity: activeCard === i ? 1 : 0.6 }}>
                <CardVisual gradient={c.gradient} labelColor={c.labelColor} textColor={c.textColor} type={c.type} cardNum={c.cardNum} user={user} accountIndex={i} />
          ...
          <div className="flex justify-center gap-2 mt-3">
            {CARDS_DEF.map((_, i) => (
              <div key={i} className="rounded-full transition-all duration-300" style={{ width: activeCard === i ? 16 : 6, height: 6, backgroundColor: activeCard === i ? '#ffffff' : 'rgba(255,255,255,0.3)' }} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white mx-4 mt-4 rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 pt-5 pb-4">
          <button className="w-full py-3 rounded-full font-bold text-sm text-white shadow-md active:scale-95 transition-transform" style={{ backgroundColor: LCL_BLUE }}>SOS Carte</button>
        </div>
        <div className="flex border-b border-gray-200 px-6">
          {['parametres', 'options'].map((tab) => (
            <button key={tab} onClick={() => setTab(tab)} className="flex-1 pb-2 text-sm font-semibold transition-colors" style={{ color: activeTab === tab ? LCL_BLUE : '#9CA3AF', borderBottom: activeTab === tab ? `2px solid ${LCL_BLUE}` : '2px solid transparent' }}>
        ...
        <div className="px-6 py-4 space-y-3">
          {activeTab === 'parametres' ? (
            <>
              <button className="w-full border border-gray-300 rounded-xl py-3 px-4 flex items-center justify-center gap-2 font-semibold text-sm"><Apple className="w-4 h-4" />Configurer Apple Pay</button>
              <div className="flex items-center justify-between bg-gray-50 rounded-xl p-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-blue-50 border-2 flex items-center justify-center" style={{ borderColor: LCL_BLUE }}><Shield className="w-5 h-5" style={{ color: LCL_BLUE }} /></div>
                  <div><p className="text-sm font-semibold text-gray-800">Verrouillage de la carte</p><p className="text-xs text-gray-500">{locked ? 'Carte verrouillée' : 'Carte déverrouillée'}</p></div>
                </div>
                <Toggle enabled={locked} onChange={setLocked} />
              </div>
              <div className="flex items-center justify-between bg-gray-50 rounded-xl p-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-blue-50 border-2 flex items-center justify-center" style={{ borderColor: LCL_BLUE }}><Wifi className="w-5 h-5" style={{ color: LCL_BLUE }} /></div>
                  <div><p className="text-sm font-semibold text-gray-800">Paiement sans contact</p><p className="text-xs text-gray-500">{nfc ? 'Activé' : 'Désactivé'}</p></div>
                </div>
            ...
            <>
              <div className="flex items-center justify-between bg-gray-50 rounded-xl p-4">
                <div><p className="text-sm font-semibold text-gray-800">Paiement en ligne</p><p className="text-xs text-gray-500">{online ? 'Activé' : 'Désactivé'}</p></div>
                <Toggle enabled={online} onChange={setOnline} />
              </div>
              <div className="flex items-center justify-between bg-gray-50 rounded-xl p-4">
                <div><p className="text-sm font-semibold text-gray-800">Paiement à l'étranger</p><p className="text-xs text-gray-500">{abroad ? 'Activé' : 'Désactivé'}</p></div>
