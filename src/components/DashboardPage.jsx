        ...
        <div className="space-y-3">
          {user?.accounts?.map((acc, i) => (
            <div key={i} className="bg-white rounded-2xl shadow-sm p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: i === 0 ? `${LCL_YELLOW}30` : `${LCL_BLUE}15` }}>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">N° {acc.accountNumber || '—'}</p>
                </div>
              </div>
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-[80%] max-w-xs bg-white shadow-2xl z-50 flex flex-col">
            <div className="p-5 text-white" style={{ backgroundColor: LCL_BLUE }}>
              ...
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-full flex items-center justify-center font-bold" style={{ backgroundColor: LCL_YELLOW, color: LCL_BLUE }}>{initials}</div>
                <div>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-1">
              {[
                ...
                <button key={item.label} onClick={() => { navigate(item.page); setShowMenu(false); }} className="w-full flex items-center gap-3 p-3 hover:bg-gray-100 rounded-xl transition">
            ...
            <div className="p-3 border-t border-gray-100">
              <button onClick={() => { setShowMenu(false); handleLogout(); }} className="w-full flex items-center gap-3 p-3 bg-red-50 hover:bg-red-100 rounded-xl transition text-red-600">
